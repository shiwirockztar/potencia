...
\vspace*{-1cm}
\begin{center}
{\Huge\textcolor{tituloazul}{\textbf{Laboratorio 3}\\Diseño, simulación y validación experimental de un convertidor Boost}}
\vspace{0.2cm}
...
La implementación completa de la etapa de potencia y del control puede observarse en la \cref{fig:circuito_boost}. Esta representación permite relacionar el esquema funcional anterior con las conexiones concretas del sensor, el driver, el interruptor y la carga.
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{\detokenize{images/simulacion/circuito_boost.png}}
\caption{Circuito Boost completo utilizado para la simulación.}
\label{fig:circuito_boost}
\end{figure}
...
Durante las pruebas de montaje se observó la señal de salida del ACS712 mostrada en la \cref{fig:acs712_out}. Esta señal constituye la medición que posteriormente se acondiciona para compararla con el setpoint. La figura permite verificar la presencia de una señal de sensado, pero no permite determinar por sí sola una corriente absoluta con precisión; para ello se requiere la escala vertical y la calibración del sensor.
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{\detokenize{images/montaje/ACS712_out.png}}
\caption{Señal de salida medida en el sensor de corriente ACS712 durante el montaje.}
\label{fig:acs712_out}
\end{figure}
...
La comparación simulada entre la señal PWM aplicada a la entrada del driver y la señal obtenida a su salida se presenta en la \cref{fig:pwm_driver_sim}. La forma de onda azul oscuro permite evaluar el acondicionamiento de amplitud y los retardos introducidos por el UCC21520 respecto a la señal cian de entrada.
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{\detokenize{images/simulacion/senal_pwm(Cian)_vs_salida_Ucc21520(DarkBlue).png}}
\caption{Comparación simulada entre la señal PWM antes del INB y la salida del UCC21520.}
\label{fig:pwm_driver_sim}
\end{figure}
...
La relación entre la señal de control, la salida del driver y la respuesta de la carga se muestra en la \cref{fig:pwm_driver_carga_sim}. La señal cian representa el PWM antes del INB, la azul oscuro la salida del UCC21520 y la verde la tensión en la carga. La separación entre estas señales permite distinguir el procesamiento de control de la respuesta dinámica del convertidor; la señal verde no debe interpretarse como una medición experimental.
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{\detokenize{images/simulacion/senal_pwm(Cian)_vs_salida_Ucc21520(DarkBlue)_vs_respuesta_salida_en carga(Green).png}}
\caption{Relación simulada entre PWM, salida del driver UCC21520 y tensión en la carga.}
\label{fig:pwm_driver_carga_sim}
\end{figure}
...
\subsection{Señal PWM generada por el ESP32}
Las mediciones de la salida PWM del ESP32 para los ciclos de trabajo de \SI{10}{\percent}, \SI{30}{\percent}, \SI{40}{\percent} y \SI{50}{\percent} se presentan en las figuras \ref{fig:pwm_esp32_10}--\ref{fig:pwm_esp32_50}. La duración relativa del nivel alto aumenta al incrementar el duty cycle, mientras la frecuencia se mantiene aproximadamente constante en las condiciones de la prueba. Estas señales corresponden a la salida del microcontrolador, antes del acondicionamiento realizado por el driver.

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{\detokenize{images/montaje/Pwm_D_10.png}}
\caption{Señal PWM medida a la salida del ESP32 con duty cycle de \SI{10}{\percent}.}
\label{fig:pwm_esp32_10}
\end{figure}

La respuesta de tensión medida en la carga para esta misma condición se muestra inmediatamente en la \cref{fig:salida_carga_10}. La pareja de figuras permite relacionar directamente el tiempo de conducción aplicado por el ESP32 con la respuesta experimental del convertidor.
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{\detokenize{images/montaje/Salida_Pwm_D_10.png}}
\caption{Tensión de salida medida en la carga con duty cycle de \SI{10}{\percent}.}
\label{fig:salida_carga_10}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{\detokenize{images/montaje/Pwm_D_30.png}}
\caption{Señal PWM medida a la salida del ESP32 con duty cycle de \SI{30}{\percent}.}
\label{fig:pwm_esp32_30}
\end{figure}

La respuesta de la carga para el mismo duty cycle se presenta en la \cref{fig:salida_carga_30}. Al comparar ambas figuras se observa la relación entre el ancho del pulso de entrada y la tensión continua obtenida en la salida.
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{\detokenize{images/montaje/Salida_Pwm_D_30.png}}
\caption{Tensión de salida medida en la carga con duty cycle de \SI{30}{\percent}.}
\label{fig:salida_carga_30}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{\detokenize{images/montaje/Pwm_D_40.png}}
\caption{Señal PWM medida a la salida del ESP32 con duty cycle de \SI{40}{\percent}.}
\label{fig:pwm_esp32_40}
\end{figure}

La tensión de salida medida para el 40\% se encuentra en la \cref{fig:salida_carga_40}. Ambas evidencias deben analizarse bajo la misma tensión de entrada, carga y tiempo de observación.
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{\detokenize{images/montaje/Salida_Pwm_D_40.png}}
\caption{Tensión de salida medida en la carga con duty cycle de \SI{40}{\percent}.}
\label{fig:salida_carga_40}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{\detokenize{images/montaje/Pwm_D_50.png}}
\caption{Señal PWM medida a la salida del ESP32 con duty cycle de \SI{50}{\percent}.}
\label{fig:pwm_esp32_50}
\end{figure}

Finalmente, la respuesta de la carga para el 50\% se presenta en la \cref{fig:salida_carga_50}. Esta última pareja completa la comparación experimental entre el duty cycle programado y la tensión de salida.
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{\detokenize{images/montaje/Salida_Pwm_D_50.png}}
\caption{Tensión de salida medida en la carga con duty cycle de \SI{50}{\percent}.}
\label{fig:salida_carga_50}
\end{figure}

La comparación de las figuras confirma cualitativamente el funcionamiento de la modulación por ancho de pulso: el intervalo de conducción aumenta desde el 10\% hasta el 50\%, mientras el período permanece esencialmente constante. Los valores exactos de frecuencia y duty deben obtenerse de los cursores del osciloscopio; las capturas por sí solas no se utilizan para declarar una precisión numérica que no sea legible.

\subsection{Señal Gate-Source del MOSFET}
Las señales de compuerta respecto a la fuente del IRFZ44N para duty cycles de \SI{30}{\percent} y \SI{40}{\percent} se muestran en las figuras \ref{fig:gate_source_30} y \ref{fig:gate_source_40}. En ambos casos, la señal conserva la naturaleza pulsada del PWM, pero su amplitud y sus tiempos de transición dependen del driver, de la resistencia de compuerta y de las características parásitas del montaje. El aumento del duty se refleja en un mayor tiempo durante el cual el MOSFET permanece excitado.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{\detokenize{images/montaje/IRFZ44N_D_30.png}}
\caption{Señal Gate-Source del IRFZ44N medida con duty cycle de \SI{30}{\percent}.}
\label{fig:gate_source_30}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{\detokenize{images/montaje/IRFZ44N_D_40.png}}
\caption{Señal Gate-Source del IRFZ44N medida con duty cycle de \SI{40}{\percent}.}
\label{fig:gate_source_40}
\end{figure}
...
\subsection{Registro conjunto de las respuestas}
Las parejas de figuras anteriores permiten comparar directamente cada señal PWM con su respuesta experimental. No se dispone de una lectura numérica suficientemente fiable para transcribir valores exactos de tensión desde las capturas; por ello, las imágenes se conservan como evidencia experimental y los valores deben registrarse posteriormente con el multímetro junto con la tensión de entrada, la carga conectada y el estado térmico del convertidor.
